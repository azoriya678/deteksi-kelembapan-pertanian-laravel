...
<body>
<div class="sidebar">
    <img src="<?php echo e(asset('images/profile.png')); ?>" alt="Profile Picture" class="profile-img">
...
<div class="content">
    <div class="status-bar">
        <span class="status">Kelembapan: <strong id="humidity">43%</strong></span>
        <span class="status">Suhu: <strong id="temperature">12°C</strong></span>
    </div>
    ...
    <div class="status-container">
        <div class="status-box">
            <h3>Kondisi Basah</h3>
            <p id="moisture-status" class="status-ok">OK</p>
        </div>
        <div class="status-box">
            <h3>Irigasi Air</h3>
...
<script>
     let humidity = Math.floor(Math.random() * 40) + 30; // Random antara 30% - 70%
    let temperature = Math.floor(Math.random() * 15) + 10; // Random antara 10°C - 25°C
    ...
    let newHumidity = Math.floor(Math.random() * 40) + 30; // Random 30%-70%
    let newTemperature = Math.floor(Math.random() * 15) + 10; // Random 10°C-25°
    let currentDate = new Date().toISOString().split('T')[0]; // Format YYYY-MM-DD
    let currentTime = new Date().toLocaleTimeString(); // Format HH:MM:SS
    ...
    fetch('/save-history', {
        ...
        body: JSON.stringify({
            tanggal: currentDate,
            jam: currentTime,
            kelembapan: newHumidity,
            suhu: newTemperature,
            status: newHumidity >= 50 ? 'Basah' : 'Kering'
        })
    })
    ...
    document.getElementById('moisture-status').textContent = newHumidity >= 50 ? 'OK' : 'Kering';
});
...
</html><?php /**PATH C:\laragon\htdocs\deteksi_kelembapan_pertanian_normal\resources\views/dashboard.blade.php ENDPATH**/ ?>